...
const reviewInitialItems: ReviewItem[] = [
  ...
  },
  {
    ...
    notificationDeliveryId: "demo-existing-4"
  },
...
export default function App() {
  ...
  const reviewSubmissionCandidates = reviewItems.filter(
    (item) => item.status === "waiting" || item.status === "reminded"
  );
  ...
  const deliverReviewNotification = async (
    ...
    if (!campaigns.some((campaign) => campaign.id === item.campaignId)) return "failed";
  ...
  const handleReviewSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const submittedItem: ReviewItem = {
      ...item,
      status: "published",
    ...
    setReviewItems((items) =>
      items.map((entry) => (entry.id === item.id ? submittedItem : entry))
    );
  ...
  const retryReviewNotification = async (item: ReviewItem) => {
  ...
  const advanceReview = (id: number) => {
    setReviewItems((items) =>
      items.map((item) => {
        ...
        if (item.status === "reminded") return { ...item, reminders: item.reminders + 1 };
  ...
  const renderReviewRecovery = () => {
    ...
    const notified = reviewItems.filter(
      (item) => item.notificationStatus === "sent" || item.notificationStatus === "simulated"
    ).length;
      ...
      <main className="review-page">
        ...
        <section className="dashboard-shell">
          ...
          <section className="review-automation" aria-label="후기 링크 등록과 사장님 알림">
            ...
            {reviewSubmitMessage ? (
              <p className="review-submit-message" role="status">{reviewSubmitMessage}</p>
            ) : null}
          ...
          <div className="metric-grid">
            ...
            <article className="is-notified"><span>사장님 알림</span><strong>{notified}</strong><small>전송 또는 데모 완료</small></article>
          </div>
                  ...
                  {filteredReviews.map((item) => {
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>
                          {item.reviewUrl ? (
                            <a className="review-link" href={item.reviewUrl} target="_blank" rel="noreferrer">
                              후기 열기
                            </a>
                          ) : (
                            <strong>{item.due}</strong>
                          )}
                          <small className="review-date-note">
                            {item.submittedAt
                        ...
                        <td>
                          <span className={`review-status is-${item.status}`}>{reviewStatusLabels[item.status]}</span>
                        ...
                        <td>
                          {item.notificationStatus === "failed" ? (
                            <button className="table-action is-retry" type="button" onClick={() => retryReviewNotification(item)}>
                              알림 재시도
                            </button>
                          ) : item.notificationStatus === "sending" ? (
                            <button className="table-action" type="button" disabled>전송 중</button>
