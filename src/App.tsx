...
const pilotAreas = [
  ...
  {
    region: "광주파일럿",
    title: "광주 동명동·충장로 맛집 방문 인증",
    description: "신규 식당의 디너 전 비는 시간대를 체험단 방문과 QR 체크인으로 채웁니다.",
    metric: "방문 인증 우선",
    imageUrl:
      "https://images.unsplash.com/photo-1528605248644-14dd04022da1?auto=format&fit=crop&w=700&q=80",
    tags: ["전라도 광주", "맛집", "노쇼 방지"]
  },
  {
    region: "부천파일럿",
    title: "부천 액티비티·문화 공간 체험단 운영",
    description: "실내 활동, 문화 공간, 클래스형 매장의 회차별 예약 슬롯을 체험단과 연결합니다.",
    metric: "회차형 슬롯",
    imageUrl:
      "https://images.unsplash.com/photo-1522163182402-834f871fd851?auto=format&fit=crop&w=700&q=80",
    tags: ["경기 부천", "액티비티", "문화"]
  }
];
...
export default function App() {
  ...
  return (
    <main className="app">
      ...
      <section className="workbench" id="캠페인" aria-label="캠페인 목록과 상세">
      ...
      <section className="pilot-section" id="파일럿" aria-label="파일럿 운영 지역">
        <div className="section-heading">
          <div>
            <p>지역 파일럿</p>
        ...
        <div className="pilot-grid">
          {pilotAreas.map((pilot) => (
            <article className="pilot-card" key={pilot.region}>
              <img src={pilot.imageUrl} alt={`${pilot.region} 대표 이미지`} />
      ...
      <section className="advertising-section" id="광고문의" aria-label="광고 문의">
        <div className="advertising-copy">
          <p>광고문의</p>
          <h2>한산 시간대 방문을 실제 후기와 연결하는 광고 상품</h2>
          <span>
        ...
        <form className="ad-inquiry-form" onSubmit={handleInquirySubmit}>
          ...
          <label className="form-field">
            ...
            <textarea
              required
              placeholder="예: 성수동 신규 오픈 매장이라 평일 오후 방문 후기를 확보하고 싶어요."
            />
          </label>
          ...
          {inquirySubmitted ? (
            <p className="application-message" role="status">
              광고 문의가 임시 접수되었습니다. 실제 연동 전까지는 화면 확인용 메시지입니다.
            </p>
          ) : null}
        </form>
      </section>
