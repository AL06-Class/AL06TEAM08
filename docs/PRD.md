...
디자인 방향은 확정되었으며, 브랜드·상태 색상·내비게이션·홈 화면·폼 화면 구성 기준은 [docs/design-system.md](design-system.md)를 따른다.

## 11. 화면 설계

홈 화면과 캠페인 등록 폼 화면 디자인이 완료되었다. 화면 구성 패턴과 상태 색상 규칙은 [docs/design-system.md](design-system.md)를 따른다.

캠페인 등록 폼 화면은 4단계 핵심기능 중 "한산시간대 예약슬롯 연동 캠페인 등록"의 구현 화면이다.

홈 화면은 방문 인증 체험단 캠페인을 탐색하고 사장님 모집 등록으로 이동시키는 진입 화면이다. 캠페인 등록 폼 화면은 사장님이 매장, 예약 연동, 한산 시간대, 모집 조건을 입력하면서 우측 프로세스 요약 패널로 전체 진행 흐름을 확인하는 입력 화면이다.
...
| 사장님 성함 | 매장 운영자 이름 |
| 연락처 | 캠페인 운영 연락처 |
| 지역 | 매장 위치 또는 캠페인 노출 지역 |
| 카테고리 | 맛집, 카페, 액티비티, 문화 등 캠페인 카테고리 |
| 예약연동 | 네이버예약 등 외부 예약 연동 여부 |
| 한산시간대 | 방문 유도가 필요한 시간대 |
| 모집인원 | 캠페인 체험단 모집 수 |
| 제공내역 | 체험단에게 제공할 혜택 |
| 캠페인목표 | 방문 후기 확보, 노쇼 감소 등 캠페인 목적 |
| 시간대분석후 자동등록 사용 여부 | 한산 시간대 분석 후 예약 슬롯을 자동 등록할지 선택하는 체크박스 |
| 선호 체험단 기준 | 지역, 카테고리, 콘텐츠 성향 등 매칭 기준 |
