...
- 현재 필요한 환경변수는 다음과 같습니다.
  ...
  - `VITE_REVIEW_NOTIFICATION_ENDPOINT`
- 카카오 알림톡 API 키, Gmail OAuth 토큰 등 발송 자격증명은 프론트엔드 환경변수에 저장하지 않습니다.
- 실제 발송은 Firebase Functions 등 서버 환경에서 처리하고 프론트엔드는 알림 엔드포인트만 호출합니다.
- `VITE_REVIEW_NOTIFICATION_ENDPOINT`가 비어 있으면 로컬 데모 발송으로 동작합니다.
...
- 2026-07-20: 후기 등록 알림 서버 엔드포인트와 카카오·Gmail 자격증명 분리 원칙 추가
