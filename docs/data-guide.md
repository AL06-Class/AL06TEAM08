...
- 제품 요구사항과 최종 DB 스키마는 `docs/PRD.md`를 우선 기준으로 사용합니다.
- 이 문서의 `bloggerCampaigns`, `campaignApplications`, `visitCheckIns` 등은 현재 React/Vite 프로토타입에서 사용한 화면 모델입니다.
- 실제 Firebase 연결 전 `stores`, `campaigns`, `applicants`, `visits` 구조로 매핑하고 PRD의 snake_case 필드 규칙에 맞춰 마이그레이션합니다.
- 마이그레이션 전까지 기존 화면 데이터 이름을 갑자기 변경해 현재 SPA 동작을 깨지 않습니다.
